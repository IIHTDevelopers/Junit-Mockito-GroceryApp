...
import com.groceryapp.test.GroceryAppTest;

public class MyApp {

	public static void main(String[] args) throws IOException {

		String sourcePath = "src/main/java/com/groceryapp/test/GroceryAppTest.java";
		String destPath = "src/test/java/com/groceryapp/test/GroceryAppTest.java";
		copyFile(sourcePath, destPath);
		Result result = JUnitCore.runClasses(GroceryAppTest.class);
		...
		try{
			...
			try (BufferedReader br = new BufferedReader(new FileReader(csvFile))) {
				...
				while ((linen = br.readLine()) != null) {
					...
					if (columns.length > 2 && columns[2].contains("GroceryManager")) {
			...
			try (BufferedReader br = new BufferedReader(new FileReader(csvFile))) {
			  ...
			  while ((linen = br.readLine()) != null) {
				  ...
				  // Check if the third column exists and contains "Ebillservice"
				  if (columns.length > 2 && columns[2].contains("ValidationUtils")) {
					  //System.out.println("Text 'Ebillservice' found in row number: " + rowNumber);
					  break;
				  }
			  }
